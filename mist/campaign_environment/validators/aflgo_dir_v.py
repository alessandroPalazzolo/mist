...
class AFLGoDirV(Validator):
    def __init__(self):
        super().__init__('aflgo_dir')

    def validate_field(self, cctx: ContextProxy) -> None:
        aflgo_dir: Path = cctx.select(self._field)

        if not aflgo_dir.is_dir():
            raise CCTXValidationError('AFLGo directory does not exist.', self._field)
        
        # FUTURE: enforce correct aflgo build
