...
class MistDirV(Validator):
    def __init__(self):
        super().__init__('mist_dir')

    def validate_field(self, cctx: ContextProxy) -> None:
        mist_dir: Path = cctx.select(self._field)

        if not mist_dir.is_dir():
            raise CCTXValidationError('Mist directory does not exist.', self._field)