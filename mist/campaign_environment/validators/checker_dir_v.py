...
class CheckerDirV(Validator):
    def __init__(self):
        super().__init__('checker_dir')

    def validate_field(self, cctx: ContextProxy) -> None:
        checker_dir: Path = cctx.select(self._field)

        if not checker_dir.is_dir():
            raise CCTXValidationError('Checker directory does not exist.', self._field)